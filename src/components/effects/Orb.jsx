...
export default function Orb({ hue = 0, hoverIntensity = 0.2, rotateOnHover = true, forceHoverState = false }) {
  ...
  const frag = /* glsl */ `
    ...
    // baseline constants; will be scaled by globalScale uniform
    ...
    vec4 draw(vec2 uv) {
      ...
      // scale radii/noise according to globalScale so orb size adapts to viewport
      float innerRadius = baseInnerRadius * globalScale;
      float noiseScale = baseNoiseScale * globalScale;
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      let width = Math.round(rect.width || container.clientWidth || window.innerWidth);
      let height = Math.round(rect.height || container.clientHeight || width || window.innerHeight);

      // If height is 0 (sometimes happens before layout settles), prefer square based on width
      if (!height || height === 0) {
        height = width || window.innerHeight;
      }

      // Ensure we have sensible non-zero sizes
      ...
      // compute a globalScale based on the smallest dimension relative to a baseline (800)
      const minSize = Math.min(gl.canvas.width, gl.canvas.height) / dpr;
      const baseline = 800; // baseline size used in shader constants
      const scale = Math.max(0.45, Math.min(1.6, minSize / baseline));
    ...
    const update = t => {
      ...
      if (isMobileDetected) {
        // On mobile, keep a subtle hover state and always rotate for a lively effect
        program.uniforms.hover.value += (1 - program.uniforms.hover.value) * 0.06;
